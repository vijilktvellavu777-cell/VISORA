...
import { emailTemplateTypeLabel } from "@/lib/email-templates";
import type { EmailTemplateRow } from "@/components/email-templates-page";
...
export function EmailTemplatePreviewModal({ template, onClose }: Props) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      ...
      <div className="relative flex max-h-[90vh] w-full max-w-5xl flex-col overflow-hidden rounded-xl border border-border bg-surface shadow-xl">
        <div className="flex items-center justify-between border-b border-border px-6 py-4">
          <div>
            <h2 className="text-lg font-semibold text-foreground">Preview: {template.name}</h2>
            <p className="mt-1 text-sm text-muted">{emailTemplateTypeLabel(template.editorType)}</p>
          </div>
          <button
            type="button"
            onClick={onClose}
          ...
          <aside className="w-full shrink-0 border-t border-border bg-surface px-6 py-6 lg:w-[280px] lg:border-l lg:border-t-0">
            <dl className="space-y-4 text-sm">
              <div>
                ...
                <dd className="mt-1 text-foreground">{template.subject ?? "—"}</dd>
              </div>
              <div>
                <dt className="font-medium text-muted">Status</dt>
                <dd className="mt-1 capitalize text-foreground">{template.status}</dd>
              </div>
              <div>
                <dt className="font-medium text-muted">Inclusion count</dt>
                <dd className="mt-1 text-foreground">{template.inclusionCount}</dd>
              </div>
              <div>
                <dt className="font-medium text-muted">Last edited</dt>
                <dd className="mt-1 text-foreground">{format(new Date(template.updatedAt), "MMM d, yyyy")}</dd>
              </div>
            </dl>
          </aside>
        </div>
